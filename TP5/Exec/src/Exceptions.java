    ...
    public static List<String> propagationDeMarqueurs(JSONObject reseauSemantique, List<String> node1, List<String> node2, String relation) {
        ...
        for (int i = 0; i < Math.min(node1.size(), node2.size()); i++) {
            ...
            try {
                ...
                for (Object o : edges) {
                    JSONObject edge = (JSONObject) o;
                    if (edge.get("to").equals(M1.get("id")) && edge.get("label").equals("is a") && !edge.containsKey("edge_type")) {
                ...
                while (!propagationEdges.isEmpty() && !solutionFound) {
                    ...
                    for (Object o : edges) {
                        JSONObject edge = (JSONObject) o;
                        if (edge.get("from").equals(tempNode.get("from")) && edge.get("label").equals(relation) && !edge.containsKey("edge_type")) {
                    ...
                    if (!solutionFound) {
                        for (Object o : edges) {
                            JSONObject edge = (JSONObject) o;
                            if (edge.get("to").equals(tempNode.get("from")) && edge.get("label").equals("is a") && !edge.containsKey("edge_type")) {
                                propagationEdges.add(edge);
                            }
                        }
                    }
